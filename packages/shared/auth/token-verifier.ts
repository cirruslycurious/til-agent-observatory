 ...
 */
export function verifyJobTokenHash(token: string, storedHash: string): boolean {
 ...
 * 
 * For hot endpoints (status query), use GetItem once and verifyTokenHash separately
 */
...
export interface GetJobAndVerifyTokenResult {
  job: JobItem | null;
  isValid: boolean;
  isExpired: boolean;
}
 ...
 */
export async function getJobAndVerifyToken(
  dynamoClient: any, // DynamoDBDocumentClient
  tableName: string,
  jobId: string,
  token: string
): Promise<GetJobAndVerifyTokenResult> {
