...
export interface BudgetState {
  ...
  estimated_cost_usd: number; // Computed at API boundary
  minimal_mode_activated: boolean;
  minimal_mode_activated_at?: string;
  tokens_used: number;
  searches_remaining: number;
  created_at: string;
  updated_at: string;
  last_cost_event_at?: string;
  last_cost_event_id?: string;
 ...
 */
export async function initializeJobBudget(jobId: string): Promise<BudgetState> {
  ...
  try {
    ...
    if (error.name === 'ConditionalCheckFailedException') {
      ...
      return getJobBudget(jobId) as Promise<BudgetState>;
    } else {
 ...
 */
export async function getJobBudget(jobId: string): Promise<BudgetState | null> {
  try {
