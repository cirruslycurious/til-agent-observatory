 ...
 */
export async function buildDependencyGraph(jobId: string): Promise<DependencyGraph> {
  // Get all artifacts for job
  const allArtifacts = await getAllArtifactsForJob(jobId);
  
  // Build artifact lookup by artifact_id
  const artifactsById: Record<string, any> = {};
  ...
  for (const artifact of allArtifacts) {
    ...
    for (const source of sources) {
      ...
      if (sourcePointer.startsWith('artifact:')) {
        ...
        if (artifactIdPart && !upstreamArtifacts.find((ua: any) => ua.artifact_id === artifactIdPart)) {
...
function detectCircularDependencies(
  ...
  const circularArtifacts = new Set<string>();
  
  function dfs(artifactId: string, path: string[]): boolean {
...
export async function getArtifactDescendants(
  ...
  const forwardGraph: Record<string, string[]> = {};
  for (const edge of graph.edges) {
    const fromId = edge.from_artifact_id;
    const toId = edge.to_artifact_id;
