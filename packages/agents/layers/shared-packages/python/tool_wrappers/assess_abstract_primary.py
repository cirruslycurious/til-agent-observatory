"""
Abstract Assessor Tool - Primary
Tool wrapper for Nova Assessor Primary Lambda

Allows Manager to invoke the primary Nova assessor and get a composite score.
...
# Configuration
ASSESSOR_PRIMARY_FUNCTION = 'til-assess-abstract-primary'
REGION = 'us-east-1'
...
def assess_abstract_primary(
    ...
    """
    Call Primary Nova Assessor to score an abstract.
    ...
    Returns:
        {
            "composite_score": 7.5,
            "assessment_id": "primary-uuid",
            "assessor": "primary"
        }
    """
    start_time = time.time()
    
    print(f"assess_abstract_primary called: conference={conference}, iteration={iteration}")
    ...
    try:
        ...
        # Invoke Primary Assessor Lambda
        response = lambda_client.invoke(
            FunctionName=ASSESSOR_PRIMARY_FUNCTION,
        ...
        # Log tool call
        log_tool_call(
            tool_name='assess_abstract_primary',
            params={
                'conference': conference,
        ...
        print(f"Primary assessor returned score: {composite_score}, elapsed: {elapsed_ms}ms")
        ...
        print(f"assess_abstract_primary error: {error_msg}")
