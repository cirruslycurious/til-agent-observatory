 ...
 */
function canonicalJsonSerialize(data) {
 ...
 */
export async function uploadArtifactToS3(jobId, taskId, artifactVersion, artifactData, compress = true, writerRequestId) {
    ...
    try {
        ...
        throw new S3UploadError(`S3 upload failed for ${s3Key}: ${errorCode} - ${errorMessage}`, error);
    }
}
/**
 ...
 */
export async function uploadToolResultsToS3(jobId, taskId, artifactVersion, toolResults, toolName, toolCallId, writerRequestId) {
    // Serialize to canonical JSON to compute size
 ...
 */
export async function downloadArtifactFromS3(s3Key, expectedHash) {
    try {
        // Download object from S3
        const response = await s3Client.send(new GetObjectCommand({ Bucket: S3_BUCKET_NAME, Key: s3Key }));
        if (!response.Body) {
        ...
        if (expectedHash) {
            ...
            if (computedHash !== expectedHash) {
                throw new HashMismatchError(`Hash mismatch for ${s3Key}: expected ${expectedHash}, got ${computedHash}`);
            }
        }
        return artifactData;
    }
        ...
        if (errorCode === 'NoSuchKey' || errorCode === 'NotFound' || errorCode === '404') {
            ...
            throw new S3DownloadError(`S3 download failed for ${s3Key}: ${errorCode} - ${errorMessage}`, error);
        }
    }
}
//# sourceMappingURL=s3_uploader.js.map